...
## Notes for members
### Notes:
1. 已经把symbol面板搬走了，点击添加按钮可以弹出该面板。
2. 多选已经完成。
3. 现在在写约束面板，但没想好约束的具体功能有几个，需要先写好Config.js
	re: 元素固定宽高设置，元素上下左右边距设置，元素与画板宽高比例设置，元素水平、竖直居中设置，（多选）多元素宽高比例设置（如：选中两个元素时，设置第二个被选中的元素 宽 是第一个被选中元素的 宽 的两倍），（多选）多元素上下左右边对齐，（多选）多元素水平、竖直居中设置

### Bugs:
1. ~~删除object的功能还需要改进，因为在输入框中按下修改键也能删除~~ Fixed
2. ~~如果完全禁止元素溢出画布，则无需裁剪画布功能。我的建议是，禁止元素完全脱离画布，但是可以溢出，保留裁剪功能（裁剪功能还是有存在意义的）。~~ Fixed
