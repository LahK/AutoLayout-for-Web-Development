let vm = new Vue({
  ...
  data: {
    ...
    focusingTool: null, // 当前 focus 的输入框所在的 tool DOM 元素，用于禁止编辑状态拖动面板
  ...
  methods: {
    ...
    inputOnBlur: function() {
      ...
      this.focusInputName = '';
      // 恢复选中 tool 可拖动状态
      if (this.focusingTool !== null) {
        this.focusingTool.draggable = true;
        this.focusingTool = null;
      }
    },
    // 当进入文字输入状态时
    ...
    inputOnFocus: function(name, event) {
      ...
      // 禁止选中 input 所在 tool 拖动
      let ele = event.target;
      while (ele.className.indexOf('tool') === -1) {
        ele = ele.parentNode;
      }
      this.focusingTool = ele;
      this.focusingTool.draggable = false;
    },
    // 工具面板 ondragStart 事件
    ...
    addConstraintMultiOnClick: function() {
      ...
      for (let i = 0; i < this.selectedObjects.length; i++) {
        if (this.selectedObjects[i] === firstObj) {
          continue;
        }
